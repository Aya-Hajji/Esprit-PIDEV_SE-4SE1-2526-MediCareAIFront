...
import { ChatbotService, DiseaseDiagnosis, SpecialtyDiagnosis } from '../../shared/services/chatbot.service';
...
@Component({
  ...
  styleUrls: ['./symptom-ai.component.css']
})
export class SymptomAiComponent implements OnInit, AfterViewChecked {
  ...
  ngOnInit(): void {
    ...
    if (this.isDoctor) {
      this.addBotMessage(
        "👨‍⚕️ Hello Doctor! Describe the patient's symptoms and I will identify the corresponding diseases."
      );
    } else {
      this.addBotMessage(
        '🤖 Hello! Describe your symptoms and I will guide you to the appropriate medical specialty.'
      );
    }
  }
  ...
  sendMessage(): void {
    ...
    if (symptomNames.length === 0) {
      this.addBotMessage('Please describe your symptoms separated by commas.\nExample: "Fever, Headache, Fatigue"');
      return;
    }
    ...
    this.chatbotService.diagnoseDoctor(symptomNames).subscribe({
      next: (diseases) => {
        ...
        if (!diseases || diseases.length === 0) {
          this.addBotMessage(`No disease found for the symptoms: ${symptomNames.join(', ')}.`);
        } else {
          this.messages.push({
            type: 'bot',
  ...
  clearChat(): void {
    this.messages = [];
    this.isTyping = false;
    this.addBotMessage(this.isDoctor ? "Chat cleared. Describe the patient's symptoms." : 'Chat cleared. How can I help you?');
  }
